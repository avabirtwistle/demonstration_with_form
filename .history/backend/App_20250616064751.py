from flask import Flask, request, Response, jsonify
from flask_cors import CORS

import time

app = Flask(__name__)
CORS(app)

_last_tray_qr = None
_last_location_qr = None

@app.route('/tray', methods=['POST'])
def handle_tray():
    global _last_tray_qr
    # Expect JSON payload: {"code": "<qr_code>"}
    data = request.get_json() or {}
    code = data.get("code")
    if not code:
        return jsonify({"error": "Missing code"}), 400
    _last_tray_qr = code
    app.logger.info(f"[SCAN] Tray scanned: {code}")
    return jsonify({"message": f"Tray scanned: {code}"}), 200

@app.route('/location', methods=['POST'])
def handle_location():
    global _last_location_qr
    # Expect JSON payload: {"code": "<qr_code>"}
    data = request.get_json() or {}
    code = data.get("code")
    if not code:
        return jsonify({"error": "Missing code"}), 400
    _last_location_qr = code
    app.logger.info(f"[SCAN] Location scanned: {code}")
    return jsonify({"message": f"Location scanned: {code}"}), 200

@app.route('/reset/tray', methods=['POST'])
def reset_tray():
    global _last_tray_qr
    _last_tray_qr = None
    return '', 204

@app.route('/reset/location', methods=['POST'])
def reset_location():
    global _last_location_qr
    _last_location_qr = None
    return '', 204

@app.route('/stream/tray')
def stream_tray():
    def event_stream():
        last_sent = None
        # SSE headers are handled by Flask Response
        while True:
            time.sleep(0.5)
            if _last_tray_qr and _last_tray_qr != last_sent:
                yield f"data: {_last_tray_qr}\n\n"
                last_sent = _last_tray_qr
    return Response(event_stream(), mimetype='text/event-stream')

@app.route('/stream/location')
def stream_location():
    def event_stream():
        last_sent = None
        while True:
            time.sleep(0.5)
            if _last_location_qr and _last_location_qr != last_sent:
                yield f"data: {_last_location_qr}\n\n"
                last_sent = _last_location_qr
    return Response(event_stream(), mimetype='text/event-stream')

if __name__ == '__main__':
    # Enable threaded so that multiple SSE streams can run concurrently
    app.run(host='0.0.0.0', port=5000, threaded=True)
