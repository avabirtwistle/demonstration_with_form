  ...
  return (
    <Box
      sx={{
        ...
        overflow: 'hidden',
        bgcolor: 'grey.100',      // <-- background color added here
      }}
    >
      {/* Sticky header */}
      <Box
      ...
      >
        ...
        >
          {allCategories.map(cat => (
            <ToggleButton key={cat} value={cat} aria-label={cat} sx={{ textTransform: 'none' }}>
              {cat}
            </ToggleButton>
          ))}
        </ToggleButtonGroup>
      ...
      <Box sx={{ overflowY: 'auto', flexGrow: 1 }}>
        <List disablePadding>
          {Object.entries(groups).map(([category, itemsInCategory]) => (
            <li key={category}>
              <ul>
                <ListSubheader sx={{ bgcolor: 'background.paper', textTransform: 'none' }}>
                  {category}
                </ListSubheader>
          ...
          {items.filter(i => selectedCategories.includes(i.category)).length === 0 && (
            <Typography variant="body2" align="center" sx={{ mt: 2, px: 2 }}>
